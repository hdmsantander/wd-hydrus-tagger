...
**Docker:** see **`docs/DOCKER.md`**. **`./start.sh docker-run-all -d`** starts tagger + hydrus-web (`--profile hydrus-web`); **`./start.sh docker-run -d`** is tagger only. On AMD ROCm hosts (`/dev/kfd`) those helpers also apply **`docker-compose.amd.yml`**. Compose mounts **`config.yaml` read-only** — **`PATCH /api/config`** updates in-memory only when the file cannot be written; set **`HYDRUS_WEB_URL`** / **`HYDRUS_API_URL`** / **`WD_TAGGER_USE_GPU`** env or edit host **`config.yaml`** to persist. Default **`HYDRUS_API_URL`** reaches Hydrus on the host via **`host.docker.internal:45869`**. Face tagging: **`pip install -e ".[face]"`** (included in Docker image); see **`docs/FACE_TAGGING.md`**.
